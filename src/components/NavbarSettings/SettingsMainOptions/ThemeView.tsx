    ...
    return (
        <>
            ...
            <ul className="grid w-full gap-3 grid-cols-2 mt-4">
                {themes.map(theme => (
                    <li key={theme} className={`${theme === activeTheme && "text-primary shadow-md border-primary"} text-gray-500 font-medium active:scale-90 transition-transform text-sm p-2 text-center border border-transparent hover:bg-accent rounded-lg cursor-pointer`}>{theme}</li>
                ))}
            </ul>
        </>
    )
