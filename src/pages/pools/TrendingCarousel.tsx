import {
  Carousel,
  CarouselContent,
  CarouselNext,
  CarouselPrevious,
} from "@/components/ui/carousel";
import TrendingCard from "./TrendingCard";
import { HiSparkles } from "react-icons/hi2";
import Autoplay from "embla-carousel-autoplay";

export default function TrendingCarousel() {
  const trendingCardData = [
    {
      title: "ZK Concentrated",
      fromImg: "/images/zksync_black.png",
      toImg:
        "data:image/png;base64,",
      amount: 3712414.6,
    },
    {
      title: "ZK - USDC.e",
      fromImg: "/images/zksync_black.png",
      toImg:
        "data:image/png;base64,",
      amount: 430379.4,
    },
    {
      title: "ETH - USDC.e",
      fromImg:
        "data:image/png;base64,",
      toImg:
        "data:image/png;base64,",
      amount: 3712414.6,
      percentage: 2.9,
    },
    {
      title: "ZK - USDC.e",
      fromImg: "/images/zksync_black.png",
      toImg:
        "data:image/png;base64,",
      amount: 430379.4,
    },
    {
      title: "ETH - USDC.e",
      fromImg:
        "data:image/png;base64,",
      toImg:
        "data:image/png;base64,",
      amount: 3712414.6,
      percentage: 2.9,
    },
  ];
  return (
    <Carousel
      plugins={[
        Autoplay({
          delay: 2000,
        }),
      ]}
    >
      <section className="flex justify-between items-center">
        <h2 className="font-medium text-primary flex gap-x-2 items-center">
          <HiSparkles />
          Trending
        </h2>

        <div className="flex gap-x-2">
          <CarouselPrevious className="static h-2 w-8 bg-primary rounded-lg cursor-pointer" />
          <CarouselNext className="static h-2 w-8 bg-primary rounded-lg cursor-pointer" />
        </div>
      </section>

      <div className="mt-2">
        <CarouselContent className="py-5">
          {trendingCardData.map(
            ({ title, fromImg, toImg, amount, percentage }, index) => (
              <TrendingCard
                key={title + index}
                title={title}
                fromImg={fromImg}
                toImg={toImg}
                amount={amount}
                percentage={percentage}
              />
            ),
          )}
        </CarouselContent>
      </div>
    </Carousel>
  );
}
